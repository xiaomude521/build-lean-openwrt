...
if [ -f "$CONFIG_GENERATE_FILE" ]; then
    # 注释掉从 board.json 读取主机名的部分
    sed -i '/if json_get_var hostname hostname; then/,/fi/s/^/#/' "$CONFIG_GENERATE_FILE"
    
    # 在设置 ttylogin 之后添加设置主机名为 OpenWrt 的命令
    sed -i '/set system.@system\[-1\].ttylogin=/a\
    \tuci -q set "system.@system[-1].hostname=OpenWrt"' "$CONFIG_GENERATE_FILE"
