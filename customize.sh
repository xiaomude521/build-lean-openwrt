...
if grep -q "hostname=" package/base-files/files/bin/config_generate; then
    # 尝试替换现有的主机名设置
    if sed -i "s/hostname='[^']*'/hostname='OpenWrt'/" package/base-files/files/bin/config_generate; then
        echo "Hostname changed to OpenWrt"
    else
        echo "Warning: Failed to modify hostname using sed, but continuing with build..."
    fi
else
    # 如果找不到 hostname 设置，添加一行
    echo "set system.@system[-1].hostname='OpenWrt'" >> package/base-files/files/bin/config_generate
    echo "Added hostname setting: OpenWrt"
fi
...
if grep -q "hostname='OpenWrt'" package/base-files/files/bin/config_generate; then
    echo "Hostname successfully changed to OpenWrt"
else
    echo "Warning: Hostname may not have been changed to OpenWrt, but continuing with build..."
    # 不退出脚本，继续执行后续步骤
fi
